...
Browser tests (`bun run test:browser`) additionally need the owner's Clerk user email in `E2E_CLERK_USER_EMAIL` — put it in `apps/server/.env` next to `CLERK_SECRET_KEY` (its consumer is the Playwright process, so neither app owns it; that file is the canonical home). Both `.env` files are loaded into the Playwright process automatically; the signed-in specs use the email with `@clerk/testing` to sign in as that user.
