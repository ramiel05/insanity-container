...
test("signs in through the real GitHub social flow", async ({ page }) => {
  await page.goto("/");
  await expect(page.getByRole("button", { name: /continue with github/iu })).toBeVisible({ timeout: 15_000 });
  await expect(page.getByRole("button", { name: /continue with google/iu })).toBeVisible();
  await page.getByRole("button", { name: /continue with github/iu }).click();
