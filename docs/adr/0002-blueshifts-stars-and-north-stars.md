...
Blueshifts have a name and optional goal. Stars have a title, completion state, creation timestamp, and exactly one owning Blueshift. Deleting a Blueshift deletes its Stars; deleting a Star removes it immediately, including any North Star designation. The initial local database may be discarded when this model changes; migration and backward-compatibility complexity are intentionally out of scope.
